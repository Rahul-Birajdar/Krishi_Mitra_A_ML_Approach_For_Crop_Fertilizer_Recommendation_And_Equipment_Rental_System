...
class WeatherService {
  ...
  Future<List<HourlyWeather>> getHourlyForecast(String cityName) async {
    final response = await http.get(Uri.parse('$BASE_URL/forecast?q=$cityName&appid=$apiKey&units=metric'));

    if (response.statusCode == 200) {
      final List<dynamic> list = jsonDecode(response.body)['list'];
      return list.take(5).map((data) => HourlyWeather.fromJson(data)).toList(); // Take the first 5 hours
    } else {
